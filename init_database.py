...
def test_database(db_path="housekeep.db"):
    ...
    try:
        ...
        # Insert a test task template
        cursor.execute("""
            INSERT INTO TaskTemplates (category, title, why, how_markdown, default_frequency_days)
            VALUES (?, ?, ?, ?, ?)
        """, ("Safety", "Test Smoke Detectors", "Ensure fire safety", "Press test button on each detector", 30))
        
        template_id = cursor.lastrowid
        print(f"✅ Created test task template with ID: {template_id}")
        
        # Insert a test task
        cursor.execute("""
            INSERT INTO Tasks (home_id, template_id, title, category, frequency_days, next_due, status, priority)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (home_id, template_id, "Test Smoke Detectors", "Safety", 30, "2024-02-15", "active", 1))
